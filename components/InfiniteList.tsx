...
interface IInfiniteListProps<T, K extends keyof T> {
  total: number
  // itemHeight: number
  bufferCount: number
  heights: number[]
...
export default function InfiniteList <T, K extends keyof T> ({
  data,
  total,
  heights,
  itemRender,
  ...
  useEffect(() => {
    ...
    let totalHeight = 0
    positions.current = heights.map(height => {
      const position = {
        height,
        top: totalHeight,
  ...
  const handleScroll = (scrollTop: number) => {
    ...
    const offset = startIdx ? positions.current[startIdx - 1].bottom : 0
    // console.log('% !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!', offset)
    setOffset(offset)
  }
  ...
  const handleHeightChange = useCallback(useThrottle(() => {
    ...
    showItemNumRef.current = Math.ceil(height / Math.min(...heights))
        ...
        <Scrollbar fitParent>
          ...
          >
            {showDataList.map(item => (
              <div key={String(item[itemKey])} id={String(item[itemKey])}>
                {itemRender(item)}
              </div>
            ))}
          </div>
